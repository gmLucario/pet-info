    ...
    let context = tera::Context::from_value(json!({
        ...
        .map_err(|e| {
            errors::ServerError::InternalServerError(format!(
                "function get_pet_health_records raised an error: {}",
                e
            ))
        })?,
    }))
    ...
    let content = templates::WEB_TEMPLATES
        ...
        .map_err(|e| {
            errors::ServerError::TemplateError(format!(
                "at /pet/health/{{pet_external_id}}/{{record_type}} endpoint the template couldnt be rendered: {}",
                e
            ))
        })?;
...
async fn pet_health_records(
    ...
    let content = templates::WEB_TEMPLATES
        ...
        .map_err(|e| {
            errors::ServerError::TemplateError(format!(
                "at /pet/health/{{pet_external_id}}/{{record_type}}/tbody endpoint the template couldnt be rendered: {}",
                e
            ))
        })?;
...
async fn add_health_record(
    ...
    .map_err(|e| {
        errors::ServerError::InternalServerError(format!(
            "function add_health_record raised an error: {}",
            e
        ))
    })?;
...
async fn delete_health_record(
    ...
    .map_err(|e| {
        errors::ServerError::InternalServerError(format!(
            "function delete_pet_health_record raised an error: {}",
            e
        ))
    })?;
